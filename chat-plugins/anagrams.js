...
exports.commands = {
    ...
    anagram: function (target, room, user) {
		...
		if (room != "casino"){
			...
			anagrams[room].anagramsinactivity = setTimeout(function(){
					...
					room.add("|raw|<div class='anagram'><center>Anagrams was ended due to inactivity.<br>Nobody guessed the correct pokemon:<b> " + anagramsPokemonName[room].anagramsPokemon + "</b></center>");
			        room.update();
				}.bind(this), 120000);
		}else{
		...
		*		}.bind(this), 1000);
        ...
        var pokemon = Tools.getTemplate(p);
            if (pokemon.num === parseInt(a)) {
				...
				room.add("|raw|<div class='anagram'><center>"+user.name + " has started a game of Anagrams. Use /guessanagram or /gan to play.<br><b>Pokemon: "+Pokemonshuffled+"</b></center></div>")
			    //room.add('|raw|<b>'+Pokemonshuffled+'</b>');;*/
				room.add("|raw|<div class='anagram'><center><b>"+user.name + "</b> has started a game of Anagrams. Use /guessanagram or /gan to play.<br><b>Pokemon: "+anagramsPokemonName[room].anagramsPokemonshuffled+"</b></center></div>")
	...
	if (!anagrams[room] || anagrams[room].anagramsgamestart == false) return this.sendReply("There are currently no Anagrams playing.");
		if(anagrams[room].anagramsguess == false){
			...
			room.add("|raw|<div class='anagram'><center>Anagrams was ended by <b>" + user.name + "</b>.</center>");
		}
	},
	...
	guessanagram: function (target, room, user) {
		...
		if(toId(target) == toId(anagramsPokemonName[room].anagramsPokemon)){
			if (room != "casino"){
				room.add("|raw|<div class='anagram'><center>Congratulations <b>" + user.name + "</b> guessed the right pokemon:<b> " + anagramsPokemonName[room].anagramsPokemon + "</b></center>");
			    anagrams[room].anagramsgamestart = false;
			    anagrams[room].anagramsguess = true;
				clearTimeout(anagrams[room].anagramsinactivity);
			}else{
